            ...
            System.Threading.Tasks.Task.Run(() =>
            {
                try
                {
                    ...
                    BeginInvoke(new Action(() =>
                    {
                        try
                        {
                            // Раньше здесь стояло ещё и «&& !wsOk»: пока сокет
                            // считается живым, открытый чат перезагружал только он.
                            // Но опрос КАЖДЫЙ тик уже померил и максимальный id, и
                            // число непрочитанных мной у собеседника — и молча
                            // выбрасывал результат, записав его как учтённый. Стоило
                            // событию «прочитано» не дойти (сокет открыт, а событий
                            // нет — случай, описанный выше про здоровье), и синие
                            // галочки не появлялись уже никогда: ни поводом, ни
                            // подстраховкой. Теперь измеренное изменение — повод
                            // перезагрузить в любом случае. Сокет остаётся быстрым
            ...
            if (!string.IsNullOrEmpty(text))
            {
                ...
                if (HasLink(text))
                {
                    ...
                    foreach (var url in FindMessageLinks(text))
                    {
                        ...
                        shown.Add(dom);
                        var row = LinkSources.MakeCard(url, innerW, bubble.BackColor);
    ...
    byte[] fileData = null, string fileName = null)
        ...
        private void MarkAsRead(int senderId)
        {
            try
            {
                ...
                // Адресат ровно один — тот, чьи сообщения я прочитал. Раньше в
                // поле адресата стоял ноль, а ноль у сервера значит «всем»: событие
                // будило каждого подключённого клиента вместо одного. Android шлёт
                // его адресно (см. markAsRead), теперь и здесь так же.
                if (affected > 0)
