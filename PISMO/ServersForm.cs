...
namespace PISMO
{
    ...
    public sealed partial class ServersForm : Form
    {
        ...
        public ServersForm()
        {
            ...
            // передавая колбэк догрузки старых сообщений (первый Attach побеждает,
            // и без этого пагинация на сервере осталась бы без обработчика).
            try { ChatScroll.Attach(_pnlServers); ChatScroll.Attach(_pnlChannels);
                  ChatScroll.Attach(_pnlMembers); ChatScroll.KillHorizontal(_pnlMessages); } catch { }
        ...
        private void RenderMessages(DataTable dt, int channel)
        {
            ...
            try
            {
                ...
                foreach (DataRow r in dt.Rows)
                {
                    ...
                    ChatScroll.EnableDoubleBufferDeep(holder);   // пузырь И все дочки — не «рвутся» при прокрутке
                    holder.Paint += (s, e) =>
                    {
                        e.Graphics.SmoothingMode = System.Drawing.Drawing2D.SmoothingMode.AntiAlias;
