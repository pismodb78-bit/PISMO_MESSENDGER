...
namespace PISMO
{
    ...
    public partial class MainForm : Form
    {
        ...
        /// <summary>Тот же самый док, что и в сайдбаре ЛС (для показа в серверном
        /// виде, где сайдбар скрыт). Возвращаем контрол, чтобы ServersForm вставил
        /// его к себе — 1-в-1, без копий.</summary>
        internal Panel VoiceDock => _voiceDock;

        /// <summary>Вернуть голосовой док обратно в сайдбар ЛС (при выходе из сервера).</summary>
        internal void RestoreVoiceDockToSidebar()
        {
            try
            {
                if (_voiceDock == null) return;
                if (_voiceDock.Parent != pnlSidebar)
                {
                    _voiceDock.Parent?.Controls.Remove(_voiceDock);
                    pnlSidebar.Controls.Add(_voiceDock);
                }
                _voiceDock.Dock = DockStyle.Bottom;
                pnlSidebar.Controls.SetChildIndex(pnlUserList, 0);
                pnlSidebar.Controls.SetChildIndex(_voiceDock, 1);
                pnlSidebar.Controls.SetChildIndex(pnlSidebarFooter, 2);
                if (_convSearchHost != null) pnlSidebar.Controls.SetChildIndex(_convSearchHost, 3);
