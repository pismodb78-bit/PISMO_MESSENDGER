        ...
        private async Task RunAsync()
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    // Зонд истинного потолка захвата: просим 120 fps. Если захват
                    // способен больше 60 — упор был в цель/пайплайн; если так же
                    // ~50 — это физический потолок обновления экрана.
                    Append("");
                    Append("— зонд потолка захвата (запрос 120 fps) —");
                    var (probe, _) = await NativeNvenc.BenchmarkCaptureAsync(working, 1080, 120, 3);
                    Append($"Потолок захвата экрана: ~{probe:0} fps");
                    if (probe >= 90)
                        Append("→ захват может больше 60 — упор был в параметры, поднимем цель.");
                    else
                        Append("→ экран реально отдаёт ~" + probe.ToString("0") + " fps. Проверь: Параметры Windows → Дисплей → Расширенные → Частота обновления = 144 Гц; ноутбук подключи к питанию (VRR/энергосбережение режет частоту).");
