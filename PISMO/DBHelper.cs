...
namespace PISMO
{
    public static class DBHelper
    {
        ...
        // ── Размер порции при заливке вложений ────────────────────────────
        private static int _chunkSize;   // 0 — ещё не спрашивали

        /// <summary>
        /// Сколько байт вложения слать за один запрос.
        ///
        /// Раньше и здесь, и на телефоне стояла догадка в 4 МБ, а перед ней —
        /// попытка залить файл ЦЕЛИКОМ одним пакетом: он уходил по сети
        /// полностью, сервер его отвергал по max_allowed_packet, и только
        /// потом начиналась настоящая дозапись. То есть большой файл шёл
        /// дважды, а индикатор всё это время просто крутился.
        ///
        /// Поэтому спрашиваем предел у самого сервера и берём половину: кроме
        /// байтов в пакет уходит текст запроса и служебные поля, впритык
        /// слать нельзя. Границы — чтобы не скатиться ни в мизерные порции,
        /// ни в кусок, который идёт минутами без признаков жизни.
        /// </summary>
        public static int ChunkSize(MySqlConnection conn)
        {
            if (_chunkSize > 0) return _chunkSize;
            long max = 0;
            try
            {
                using var c = new MySqlCommand("SELECT @@max_allowed_packet", conn);
                max = Convert.ToInt64(c.ExecuteScalar());
            }
            catch { }
            long half = max > 0 ? max / 2 : 1L * 1024 * 1024;
            _chunkSize = (int)Math.Clamp(half, 256L * 1024, 4L * 1024 * 1024);
            return _chunkSize;
        }
