...
namespace PISMO.Native
{
    ...
    internal sealed class WgcCapturer : IDisposable
    {
        ...
        public WgcCapturer(IntPtr hwnd)
        {
            try
            {
                ...
                // 2) Элемент захвата для HWND через interop-фабрику активации
                //    (детерминированно: RoGetActivationFactory + CreateForWindow).
                ...
                IntPtr itemPtr = factory.CreateForWindow(hwnd, ref itemIid);
                try { _item = MarshalInspectable<GraphicsCaptureItem>.FromAbi(itemPtr); }
                finally { if (itemPtr != IntPtr.Zero) Marshal.Release(itemPtr); }
                if (_item == null) throw new InvalidOperationException("CreateForWindow вернул null");
