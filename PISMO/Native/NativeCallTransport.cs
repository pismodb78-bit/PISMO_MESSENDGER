...
namespace PISMO.Native
{
    ...
    public sealed class NativeCallTransport : IDisposable
    {
        ...
        // сигнал по порогу, а RNNoise затем чистит то, что прошло. Тише порога и после
        // hangover — глушим в ноль. Порог в dBFS относительно full-scale.
        private void ApplyVoiceGate(byte[] data, int offset, int len)
        {
            ...
            if (_voiceGateHang <= 0) Array.Clear(data, offset, len);       // ниже порога — тишина
        }
        ...
        //   • 60..100% — плюс клик-гейт/лимитер против клавиатуры/выдува.
        // Spectral.Strength/Floor тянутся по всей шкале, поэтому разница слышна на
        // всём ходу ползунка (а не только при выкл). Без wet/dry-микса: RNNoise
        // буферизует кадры (задержка), и микс «сухого» с задержанным «мокрым» ломал
        // фазу (артефакты). Тот же тракт зеркалит тест микрофона (MicTestForm).
        private void DenoiseInPlace(byte[] data, int offset, int len)
        {
            ...
            // ТОЛЬКО RNNoise. Это purpose-built нейросетевой шумодав (тот же, что в
            // Discord): чистит фон и клавиатуру, имеет собственный мягкий VAD-гейт и
            // плавный вход, звучит натурально. Сила = сухой/мокрый микс ВНУТРИ него
            // (фазово-корректный). Наслоение спектрального/гейта/HPF поверх раньше и
            // давало «рацию из Второй мировой» — оно убрано.
            var rn = _rnnoise;
            ...
            // Fallback, только если RNNoise не поднялся: МЯГКИЙ спектральный шумодав
            // (без агрессии — иначе «музыка»). Не наслаиваем гейты/HPF.
            if (_spectral != null)
            {
                _spectral.Strength = 0.4f + f * 1.4f;         // мягко (0.4 … 1.8)
                _spectral.Floor    = 0.18f - f * 0.08f;       // не глубже 0.10 — без «музыки»
        ...
        private void ProcessAndSendFrame(byte[] data, int offset, int len)
        {
            ...
            try
            {
                ...
                // 2) Результат AEC → managed для дальнейшей обработки.
                Marshal.Copy(buf, data, offset, len);
                ...
                // 5) Ручная громкость (ползунок) — финальным масштабом, после AGC.
        ...
        public void ChangeScreenCodecLive(string codec)
        {
            ...
            try
            {
                // Простая и предсказуемая логика: смена кодека = ВЫКЛючить демку, пауза
                // 3 секунды, ВКЛючить с новым кодеком. Пауза даёт SFU/сессии полностью
                // отпустить старый видео-трек (сессия «залипает» на кодеке первого
                // трека — Pion MediaEngine игнорирует другой кодек, пока старый жив),
                // чтобы новый трек согласовался как свежий. Плитка у зрителя на эти ~3с
                // пропадёт и вернётся сама (проактивное объявление при публикации).
                StopScreenShare();
