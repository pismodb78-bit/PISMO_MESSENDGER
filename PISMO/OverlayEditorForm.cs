...
namespace PISMO
{
    ...
    internal sealed class OverlayEditorForm : DarkToolWindow
    {
        ...
        private OverlayEditorForm() : base("Настройка оверлея", new Size(400, 620))
        {
            ...
            TopMost = true;
            // Окно открываем у ЛЕВОГО края. Сама плашка по умолчанию стоит у правого
            // края по центру — открываясь справа, редактор просто накрывал её собой,
            // и выглядело это как «плашка не появляется» (в звонке редактор
            // вызывался из панели звонка в другом месте, поэтому там всё работало).
            var wa = Screen.PrimaryScreen?.WorkingArea ?? new Rectangle(0, 0, 1920, 1080);
            Location = new Point(wa.Left + 40, Math.Max(wa.Top, wa.Top + 60));
